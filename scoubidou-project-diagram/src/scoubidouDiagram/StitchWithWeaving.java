...
public class StitchWithWeaving {
	...
	public void dataOfStitch(int firstLineEndPoint, int crissNumberOfLines,
			int a, int b) {
	 ...
	 */
	public void printPicture(int firstLineEndPoint, int crissNumberOfLines,
			int a, int b) throws IOException {
		int width = 2 * a * MyPrintCanvasDiagram.length + 2
				* MyPrintCanvasDiagram.x1 + 2
				* MyPrintCanvasDiagram.lengthOfStringEnd;
		int height = 2 * b * MyPrintCanvasDiagram.length + 2
				* MyPrintCanvasDiagram.y1;
		...
		BufferedImage crissImg = new BufferedImage(width, height,
				BufferedImage.TYPE_INT_ARGB);
		...
		canv.setIsPaintRectangle(false);
	 	BufferedImage crossImg = new BufferedImage(width, height,
				BufferedImage.TYPE_INT_ARGB);
	...
	/*	try { 
		 	ImageIO.write(horizoEven, "png", new File("c://temp//" +
			 "horizoEven"+ "_" + firstLineEndPoint + "_" + crissNumberOfLines + "_"
			 + a + "X" + b + ".png")); 
		 	} catch (IOException e){};
		 	
			try { 
			 	ImageIO.write(horizoOdd, "png", new File("c://temp//" +
				 "horizoOdd"+ "_" + firstLineEndPoint + "_" + crissNumberOfLines + "_"
				 + a + "X" + b + ".png")); 
			 	} catch (IOException e){};
			 	try { 
				 	ImageIO.write(paralelOdd, "png", new File("c://temp//" +
					 "paralelOdd"+ "_" + firstLineEndPoint + "_" + crissNumberOfLines + "_"
					 + a + "X" + b + ".png")); 
				 	} catch (IOException e){};*/
		...
		for (int j = 0; j < paralel.length; j = j + 2) {					
				croppedSegments[croppedSegmentIndex] = CropImage.croppedIntersect(horizoEven,
						paralel[j]);
				croppedSegmentIndex++;
			
		}
		for (int j = 1; j < paralel.length; j = j + 2) {					
			croppedSegments[croppedSegmentIndex] = CropImage.croppedIntersect(horizoOdd,
					paralel[j]);
	/*		try { 
			 	ImageIO.write(croppedSegments[croppedSegmentIndex], "png", new File("c://temp//" +
				 "croppedSegments"+croppedSegmentIndex+ "_" + firstLineEndPoint + "_" + crissNumberOfLines + "_"
				 + a + "X" + b + ".png")); 
			 	} catch (IOException e){};	*/	
			croppedSegmentIndex++;
		...
		try {
			ImageIO.write(result, "png", new File("c://temp//"
					+ "result" + "_" + firstLineEndPoint + "_"
