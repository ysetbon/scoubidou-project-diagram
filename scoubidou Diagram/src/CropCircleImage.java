...
public class CropCircleImage {
	 ...
	 * @param width
	 * @param height
	 * @return cropped image with with a transparent circle, the invert of the circle is not transparent
	 * @throws IOException
	 */
	...
	// Create a buffered image in which to draw
		...
		int[][][]  pixelImage =getPixels(bi);
		
			for (int x = 0; x < bi.getWidth(); x++) {
				for (int y = 0; y< bi.getHeight(); y++) {
				if (isInside(circle_x, circle_y, radius, x, y) == true) {
					
					//set the pixel value
				   int p = (pixelImage[y][x][3]<<24) | (pixelImage[y][x][0]<<16) | ((pixelImage[y][x][1]<<8) | pixelImage[y][x][2]);
				   ret.setRGB(x, y, p);
				}
			}
		}
	...
	public static void main(String[]args){
		...
		try {
			...
			ret = croppedIntersect(bi, 50, 50,80);
