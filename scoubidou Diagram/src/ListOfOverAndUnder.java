...
/**This class is a static class for finding the intersection points that will need to be cover in a transparent circle
 * 
 * @author User
 *
 */
 ...
 */
	public static Point[] findIntersectAllPointsToErase(nodeLine[] lines , nodeLine[] contraLines){
		int numberOfPointsToErase = (lines.length*contraLines.length)/2;
		Point[] result = new Point[numberOfPointsToErase];
		int k=0;
		for(int i=0; i<lines.length; i++){
			Point[] intersectedPointsToErase = findIntersectCertainPointsToErase(lines[i], contraLines);
	 ...
	 */
	public static Point[] findIntersectCertainPointsToErase(nodeLine l ,nodeLine[] contraLines){
	...
	/**
	 * 
	 * @param l - the single line
	 * @param contraLines - the array of lines 
	 * @return - all points that intersected l and contraLines
	 */
	public static Point[] findIntersectCertainPoints(nodeLine l , nodeLine[] contraLines){
		...
		for(int i=0; i<contraLines.length;i++){
			intersectedPoints[i]= intersection(
	...
	//Note this is intentialy a simple algorithm, many faster options are out there
	/**
	 * 
	 * @param thisPoint - the point to be compared
	 * @param listToSearch - list of points to be sorted by the distance of thisPoint
	 * @return
	 */
